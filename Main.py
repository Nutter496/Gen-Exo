...
# stellar type, etc)
#==================================================
print "Generate new stellar population data (s) or new planetary data (p)?"
l = raw_input()
if l == 's':
	fstar1 = open('Stars.dat','w')
	fstar1.write('ID\t' + 'Mass\t' + '[Fe/H]\t' + 'Radius\t' + 'Type\t' + 'Teff\t' + 'Tms\t' + 'Nplan\t' + '\n')
	f2 = open('Nplan_per_star.dat','r')	# Data from Dressing & Charbonneau 2013
	row = f2.readlines()
	cum_freq = row[-1].split( )[-1]
	Nplan_bins = [0,int(row[1].split()[-1]),int(row[2].split()[-1]),int(row[3].split()[-1]),int(row[4].split()[-1]),int(row[5].split()[-1])]
	f3 = open('Type.dat','r')		# Type categorisedby masses
	...
	with open('Stellar_Popl.dat') as f1:		# Data file with mass and metallicity for a stellar population
		for line in f1:
			...
			for i in range(len(planet_count)):
				if Nplan_bins[i] < RaNdOm <= Nplan_bins[i+1]:	
					star.Nplan = row[i+1].split()[0]	
					if (i+1) == int(star.Nplan):
						planet_count[i]=planet_count[i]+1
